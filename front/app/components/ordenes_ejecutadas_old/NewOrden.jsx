...
const App = () => {
  ...
  const [l_produccion, setLProduccion] = useState([]);
  ...
  const buscarLineaProduccion = async () => {
    const lineas = JSON.parse(list_data?.acondicionamiento.linea_produccion);
    const nuevasLineas = [...l_produccion]; // copia actual del estado
  
    for (const item of lineas) {
      const linea = await getManuId(item);
  
      // Verifica que no exista ya en el array auxiliar
      if (!nuevasLineas.some((l) => l.id === linea.id)) {
        nuevasLineas.push(linea);
      }
    }
  
    setLProduccion(nuevasLineas); // actualiza el estado una sola vez
  };
