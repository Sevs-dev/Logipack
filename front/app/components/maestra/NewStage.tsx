...
function NewStage({ canEdit = false, canView = false }: CreateClientProps) {
  ...
  const [availableActivities, setAvailableActivities] = useState<
    { id: number; description: string; binding: number; duration: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (["Actividades", "Control", "Procesos"].includes(phaseType)) {
      ...
      setSelectedActivities([]); // ← mantiene limpio al cambiar de tipo
    }
  }, [phaseType]);
  ...
  const handleEdit = async (id: number) => {
    try {
      ...
      // ESTE era el sospechoso: si viene null rompe <input type="number">
      setDurationUser(data.duration_user ?? "");

      // roles como antes, pero defensivo
        ...
        <div className="flex justify-center space-x-2 mb-2">
          <Button
            onClick={() => {
              ...
              setEditingStage(null); // *** clave ***
              setIsOpen(true); // abre modal de crear
            }}
            variant="create"
            label="Crear Fase"
          />
