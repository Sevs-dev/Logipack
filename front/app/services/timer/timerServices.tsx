...
export const createTimer = async (
  data: TimerData
): Promise<TimerResponse | { exists: true }> => {
  try {
...
export const finishTimer = async (payload: {
  ejecutada_id: number;
  pause_time: number;
}) => {
  try {
...
export const resetTimer = async (payload: {
  ejecutada_id: number;
  time_reset: number;
...
export const getTimerEjecutadaById = async (
  ejecutada_id: number
): Promise<{ exists: boolean; timer: TimerResponse | null }> => {
  try {
...
export const getcontrolTimer = async <T = unknown,>(
  timerId: number
): Promise<T[]> => {
  try {
    const { data } = await apiTimer.get(`/getFaseTimer/control/${timerId}`);
    const fases = Array.isArray(data?.maestra_fases_fk?.[0])
      ? data.maestra_fases_fk[0]
      : [];
    return fases as T[];
  } catch (error: unknown) {
    throw new Error(
